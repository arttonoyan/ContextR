...
public static class ContextRequiredExtensions
{
    ...
    /// <summary>
    /// Gets the ambient context value of type <typeparamref name="TContext"/> for the specified domain
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="accessor"/> is <see langword="null"/>.</exception>
    /// <exception cref="InvalidOperationException">Thrown when the required context is not available.</exception>
    public static TContext GetRequiredContext<TContext>(this IContextAccessor accessor, string domain)
        where TContext : class
    {
        ArgumentNullException.ThrowIfNull(accessor);

        return accessor.GetContext<TContext>(domain)
            ?? throw new InvalidOperationException(
                $"Required context '{typeof(TContext).FullName}' is not available in domain '{domain}'.");
    }
    ...
    /// <summary>
    /// Gets the captured context value of type <typeparamref name="TContext"/> for the specified domain
    /// and throws when missing.
    /// </summary>
    /// <typeparam name="TContext">The context type.</typeparam>
    /// <param name="snapshot">The context snapshot.</param>
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="snapshot"/> is <see langword="null"/>.</exception>
    /// <exception cref="InvalidOperationException">Thrown when the required context is not present in the snapshot.</exception>
    public static TContext GetRequiredContext<TContext>(this IContextSnapshot snapshot, string domain)
        where TContext : class
    {
        ArgumentNullException.ThrowIfNull(snapshot);

        return snapshot.GetContext<TContext>(domain)
            ?? throw new InvalidOperationException(
                $"Required context '{typeof(TContext).FullName}' is not available in domain '{domain}' of the captured snapshot.");
    }
}
